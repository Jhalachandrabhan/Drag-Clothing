...
import { LayoutDashboard, Users, Package, Zap, BarChart3, Box, LogOut, ShoppingCart } from 'lucide-react';
...
const ClientLayout = () => {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/client/dashboard', icon: <LayoutDashboard size={18} /> },
    { name: 'Incoming Orders', path: '/client/orders', icon: <ShoppingCart size={18} /> },
    { name: 'Product Stock', path: '/client/inventory', icon: <Box size={18} /> },
    { name: 'My Products', path: '/client/products', icon: <Package size={18} /> },
    { name: 'Store Manager', path: '/client/managers', icon: <Users size={18} /> },
    { name: 'Flash Sale', path: '/client/discounts', icon: <Zap size={18} /> },
    { name: 'Business Reports', path: '/client/reports', icon: <BarChart3 size={18} /> },
  ];
    ...
    <div className="flex h-screen bg-[#FAFAFA] font-sans text-[#1A1A1A] selection:bg-[#9B4819] selection:text-white relative">
      <aside className="w-72 bg-[#0A0A0A] text-white flex flex-col z-30 relative overflow-hidden">
        ...
        <div className="p-8 border-b border-white/5 relative z-10">
          <Link to="/client/dashboard" className="text-2xl font-black tracking-tighter text-white group">
            {user?.brandName || 'BRAND'}<span className="text-[#9B4819] group-hover:animate-pulse">.</span>
          </Link>
          <p className="text-[9px] font-bold text-white/20 uppercase tracking-[0.5em] mt-3 italic">Brand Control Unit</p>
        </div>
        ...
        <nav className="flex-1 p-6 space-y-1 overflow-y-auto relative z-10 custom-scrollbar">
          {navItems.map((item) => (
            <Link key={item.path} to={item.path} className={`flex items-center gap-4 px-5 py-4 rounded-xl transition-all duration-500 group relative overflow-hidden ${isActive(item.path) ? 'text-white' : 'text-white/30 hover:text-white'}`}>
              {isActive(item.path) && (
                <motion.div layoutId="activeGlow" className="absolute inset-0 bg-gradient-to-r from-[#9B4819]/20 to-transparent border-l-2 border-[#9B4819] z-0"/>
              )}
           ...
           <div className="flex items-center gap-6">
              <button onClick={() => setIsProfileOpen(true)} className="flex items-center gap-3 text-left hover:opacity-80 transition-opacity group">
                <div className="text-right">
                   <p className="text-[10px] font-black uppercase tracking-tight leading-none text-[#111]">{user?.name || 'Client Admin'}</p>
                   <p className="text-[8px] font-bold text-[#9B4819] uppercase tracking-widest mt-1">{user?.brandName || 'Brand Manager'}</p>
                </div>
